...
| | Connection gone stale in the writer thread | `close_old_connections()` before every flush; close the connection after 60 s idle rather than holding one forever, and once, unconditionally, when the writer thread stops (Phase 6, DECISIONS.md p06-implement/T7). |
